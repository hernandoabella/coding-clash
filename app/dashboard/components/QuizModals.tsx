...
const QuizModals = ({ onGameStart }: QuizProps) => {
  ...
  const handleStart = async () => {
    ...
    try {
      // Create a new game session using the correct /api/quizGame/start endpoint
      const response = await fetch('/api/quizGame/start', {
        method: 'POST',
        headers: {
  ...
  const handleClose = () => {
    ...
    if (selectedLanguage && selectedDifficulty && session?.user?.id) {
      // Use the correct abandon endpoint
      fetch('/api/quizGame/abandon', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
